...
import cartManager from "../controllers/cart-controller";
...
cartRouter.get('/:id/buy', async (req:Request, res:Response, next:NextFunction) =>{
  try {
    const idParams = req.params.id;
    const dataJson = await cartManager.buyCart(idParams);
    res.status(200).json({
      msg: 'Compra finalizada con exito'
    });
  } catch (error) {
    next(error);
  }
